...
- $A$ 为账户集合
- $T$ 为交易集合
- $\tau: T \rightarrow U \times U$ 为交易映射函数
- $\sigma: A \rightarrow \mathbb{R}^+$ 为余额函数
- $\rho: T \times A \rightarrow \mathbb{R}$ 为交易影响函数
- $\delta: T \rightarrow \{pending, completed, failed\}$ 为状态函数

**定义 1.2** (交易)
交易是一个五元组：
$$t = (id, sender, receiver, amount, timestamp)$$
...
- $status \in \{active, frozen, closed\}$ 为账户状态
